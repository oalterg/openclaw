...
import {
  ...
  withPermittedSessionWritesDuringPromptRelease,
...
describe("permitted writes during prompt release", () => {
  it("allows trusted in-turn writers via withPermittedSessionWritesDuringPromptRelease without triggering takeover", async () => {
    const sessionFile = await createTempSessionFile();
    const releases: string[] = [];
    const acquireSessionWriteLock = vi
      .fn()
      .mockResolvedValueOnce({ release: vi.fn(async () => releases.push("prep")) })
      .mockResolvedValueOnce({ release: vi.fn(async () => releases.push("post")) });
    ...
    // Trusted in-turn write (self-config, MCP owner tool result delivery, etc.)
    await withPermittedSessionWritesDuringPromptRelease("test-self-config", async () => {
      await fs.appendFile(sessionFile, '{"type":"self-config-mutation"}\n', "utf8");
    });

    // Subsequent normal post-prompt write should still succeed (no takeover)
    await controller.withSessionWriteLock(async () => {
      await fs.appendFile(sessionFile, '{"type":"post-permitted"}\n', "utf8");
    });
  ...
  it("still detects takeover for unpermitted writes after prompt release", async () => {
    const sessionFile = await createTempSessionFile();
    const releases: string[] = [];
    const acquireSessionWriteLock = vi
      .fn()
    ...
    // Direct mutation without the permitted wrapper should be treated as takeover
    await fs.appendFile(sessionFile, '{"type":"unpermitted-external"}\n', "utf8");
  ...
  it("supports nested permitted write contexts", async () => {
    const sessionFile = await createTempSessionFile();
    const acquireSessionWriteLock = vi.fn().mockResolvedValueOnce({
      release: vi.fn(async () => {}),
    });
    ...
    await withPermittedSessionWritesDuringPromptRelease("outer", async () => {
      await fs.appendFile(sessionFile, '{"type":"outer"}\n', "utf8");
      await withPermittedSessionWritesDuringPromptRelease("inner", async () => {
        await fs.appendFile(sessionFile, '{"type":"inner"}\n', "utf8");
      });
    });
